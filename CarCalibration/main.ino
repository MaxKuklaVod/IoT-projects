...
const uint8_t L_DIR_PIN = 7;

SoftwareSerial bluetooth(2, 3);

// --- Системные константы ---
const int   INITIAL_SPEED = 200;
const int   T_STEP_90     = 400;
const int   T_STEP_180    = 800;
const int   T_STEP_270    = 1200;
const int   T_STEP_360    = 1600;
const uint8_t STORAGE_KEY = 0xAB;
...
void drive(bool l_dir, int l_spd, bool r_dir, int r_spd) {
...
void execMove(MoveType m) {
  drive(dir_map_l[m], pwm_l, dir_map_r[m], pwm_r);
}

// --- Работа с энергонезависимой памятью ---
void saveSettings() {
  EEPROM.write(ADDR_FWD_L,   (uint8_t)dir_map_l[MOVE_FWD]);
  EEPROM.write(ADDR_FWD_R,   (uint8_t)dir_map_r[MOVE_FWD]);
  EEPROM.write(ADDR_BWD_L,   (uint8_t)dir_map_l[MOVE_BWD]);
  EEPROM.write(ADDR_BWD_R,   (uint8_t)dir_map_r[MOVE_BWD]);
  EEPROM.write(ADDR_L_ROT_L, (uint8_t)dir_map_l[MOVE_ROT_L]);
  EEPROM.write(ADDR_L_ROT_R, (uint8_t)dir_map_r[MOVE_ROT_L]);
  EEPROM.write(ADDR_R_ROT_L, (uint8_t)dir_map_l[MOVE_ROT_R]);
  EEPROM.write(ADDR_R_ROT_R, (uint8_t)dir_map_r[MOVE_ROT_R]);
  EEPROM.write(ADDR_VAL_L,   (uint8_t)pwm_l);
  EEPROM.write(ADDR_VAL_R,   (uint8_t)pwm_r);
  
  for (int i = 0; i < 4; i++) {
    int cell = ADDR_TIME_BASE + i * 2;
    EEPROM.write(cell,     (uint8_t)(rotation_intervals[i] >> 8));
    EEPROM.write(cell + 1, (uint8_t)(rotation_intervals[i] & 0xFF));
  }
...
void loadSettings() {
  ...
  for (int i = 0; i < 4; i++) {
    int cell = ADDR_TIME_BASE + i * 2;
    rotation_intervals[i] = ((uint16_t)EEPROM.read(cell) << 8) | EEPROM.read(cell + 1);
  }
}
...
void processSerial(char cmd) {
  if (active_mode == CALIBRATE_DIR) {
    if (cmd == 'F')      drive(PIN_STATES[ptr_combo][0], pwm_l, PIN_STATES[ptr_combo][1], pwm_r);
      ...
      brakes();
      if (active_move < MOVE_ROT_R) { active_move = (MoveType)(active_move + 1); ptr_combo = 0; }
    }
    else if (cmd == 'R') { brakes(); if (active_move < MOVE_ROT_R) { active_move = (MoveType)(active_move + 1); ptr_combo = 0; } }
    else if (cmd == 'C') { brakes(); if (active_move > MOVE_FWD)   { active_move = (MoveType)(active_move - 1); ptr_combo = 0; } }
    else if (cmd == 'T') { brakes(); active_mode = BALANCE_SPEED; focus_left = true; }
  } 
    ...
    else if (cmd == 'T') { brakes(); active_mode = CALIBRATE_TIME; ptr_time = 0; }
  }
    ...
    else if (cmd == 'R') rotation_intervals[ptr_time] = constrain((int)rotation_intervals[ptr_time] + 50, 50, 9999);
    else if (cmd == 'L') rotation_intervals[ptr_time] = constrain((int)rotation_intervals[ptr_time] - 50, 50, 9999);
    else if (cmd == 'S') { if (ptr_time < 3) ptr_time++; }
    else if (cmd == 'X') {
       const uint16_t defaults[4] = {T_STEP_90, T_STEP_180, T_STEP_270, T_STEP_360};
       rotation_intervals[ptr_time] = defaults[ptr_time];
    }
    else if (cmd == 'C') { if (ptr_time > 0) ptr_time--; }
    else if (cmd == 'T') { brakes(); saveSettings(); active_mode = ACTIVE_RUN; }
  }
...
void setup() {
  ...
  if (EEPROM.read(ADDR_STATUS) == STORAGE_KEY) {
    ...
    Serial.println(F("No config. CALIBRATION mode."));
  }
  displayInfo();
}
